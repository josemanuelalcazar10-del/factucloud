...
function Login({ onLogin }) {
  ...
  return (
    <div translate="no" style={{ minHeight: "100vh", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Inter','Segoe UI',Arial,sans-serif" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@300;400;500;700&display=swap');`}</style>
...
function Agente({ setFacturas, facturas, clientes, obras, proveedores, setProveedores }) {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (url.includes("access_token")) {
          ...
          if (token) { setGmailToken(token); try { localStorage.setItem("fc_gmail_token", token); } catch {} popup.close(); clearInterval(interval); }
        }
      } catch {}
  ...
  const procesarFacturasGmail = async () => {
    ...
    try {
      ...
      for (const msg of nuevos.slice(0, 10)) {
        ...
        if (!pdfPart) continue;
        const attData = await (await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/messages/${msg.id}/attachments/${pdfPart.body.attachmentId}`, { headers: { Authorization: `Bearer ${gmailToken}` } })).json();
        const pdfBase64 = attData.data.replace(/-/g, "+").replace(/_/g, "/");
        const claudeRes = await fetch("/api/anthropic", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-5", max_tokens: 1000, messages: [{ role: "user", content: [{ type: "document", source: { type: "base64", media_type: "application/pdf", data: pdfBase64 } }, { type: "text", text: "Extrae los datos de esta factura y responde SOLO en JSON: {proveedor, cif, concepto, base, iva, total, fecha, numeroFactura}. Solo el JSON." }] }] }) });
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 3, padding: "20px" }}>
              ...
              <div style={{ marginBottom: 12 }}>
                {!gmailToken
                  ? <button onClick={conectarGmail} style={{ width: "100%", background: "#f0a500", color: "#1e293b", border: "none", padding: "12px", cursor: "pointer", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace", fontWeight: "bold", borderRadius: 2, marginBottom: 8 }}>📧 Conectar Gmail</button>
                  : <button onClick={procesarFacturasGmail} disabled={procesando} style={{ width: "100%", background: procesando ? "#94a3b8" : "#4caf7d", color: "#f8f9fa", border: "none", padding: "12px", cursor: procesando ? "default" : "pointer", fontSize: 10, letterSpacing: 3, textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace", fontWeight: "bold", borderRadius: 2, marginBottom: 8 }}>{procesando ? "⏳ Procesando..." : "📧 Procesar facturas Gmail"}</button>
                }
                {gmailToken && <div style={{ fontSize: 9, color: "#4caf7d", fontFamily: "'JetBrains Mono', monospace", textAlign: "center" }}>✓ Gmail conectado</div>}
              </div>
              <div style={{ borderTop: "1px solid #f1f5f9", paddingTop: 12 }}>
              {!activo
                ? <button onClick={iniciar} style={{ width: "100%", background: "#1e293b", color: "#f8f9fa", border: "none", padding: "14px", cursor: "pointer", fontSize: 11, letterSpacing: 4, textTransform: "uppercase", fontFamily: "'JetBrains Mono', monospace", fontWeight: "bold", borderRadius: 2 }}>INICIAR DEMO</button>
...
export default function FactuCloudApp() {
  ...
  return (
    <div translate="no" style={{ minHeight: "100vh", background: "#f8fafc", color: "#1e293b", fontFamily: "'Plus Jakarta Sans', 'Inter', 'Segoe UI', Arial, sans-serif" }}>
      ...
      <style>{`* { box-sizing: border-box; } body { font-family: 'Inter','Segoe UI',Arial,sans-serif; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: #f8fafc; } ::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 3px; } input,select,textarea { font-family: inherit; } @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@300;400;500;700&display=swap'); button { font-family: inherit; }`}</style>
    </div>
  );
}
